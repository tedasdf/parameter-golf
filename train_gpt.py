...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_res,q_gain,smear,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda",
    ).split(",")
    if pattern
)
...
class FullAttnRes(nn.Module):
    ...
    def forward(self, sources: list[Tensor]) -> Tensor:
        if not sources:
            raise ValueError("FullAttnRes requires at least one source tensor")
        v = torch.stack(sources, dim=0)  # [Lsrc, B, T, D]
        k = self.norm(v)
        logits = torch.einsum("d,lbtd->lbt", self.w.to(dtype=k.dtype), k).float()
        attn = logits.softmax(dim=0).to(dtype=v.dtype)
        return torch.einsum("lbt,lbtd->btd", attn, v)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn_res_attn = FullAttnRes(dim)
        self.attn_res_mlp = FullAttnRes(dim)
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0

    def forward(
        self,
        depth_sources: list[Tensor],
        q_w: Tensor,
        k_w: Tensor,
        v_w: Tensor,
        out_w: Tensor,
        ...
        h_attn = self.attn_res_attn(depth_sources)
        attn_out, raw_v = self.attn(
            self.attn_norm(h_attn) * self.ln_scale_factor,
            q_w,
            k_w,
            v_w,
            out_w,
        ...
        depth_sources.append(attn_out)

        h_mlp = self.attn_res_mlp(depth_sources)
        mlp_out = self.mlp(
            self.mlp_norm(h_mlp) * self.ln_scale_factor,
            up_w,
            down_w,
        )
        depth_sources.append(mlp_out)
        return mlp_out, raw_v
...
class GPT(nn.Module):
    ...
    def _forward_hidden(self, input_ids: Tensor) -> tuple[Tensor, Tensor | None]:
        n = self.num_layers
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=args.compile_fullgraph)
    with torch.inference_mode():
...
def main() -> None:
    ...
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=args.compile_fullgraph)
    torch.cuda.synchronize()
