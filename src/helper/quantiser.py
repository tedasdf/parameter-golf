...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], quant_cfg: QuantConfig) -> tuple[dict[str, object], dict[str, int]]:    # Single supported clean-script export format:
    ...
    for name, tensor in state_dict.items():
        ...
        if t.numel() <= quant_cfg.keep_float_max_numel:
            kept = keep_float_tensor(name, t, passthrough_orig_dtypes, quant_cfg)
            passthrough[name] = kept
            stats["int8_payload_bytes"] += tensor_nbytes(kept)
            continue

        stats["num_float_tensors"] += 1
        q, s = quantize_float_tensor(t ,quant_cfg)
        if s.ndim > 0:
